...
@onready var player3 = get_tree().get_first_node_in_group("gun")
...
func _handle_picked_up():
	if player3.has_method("increase_ammo"):
		player3.increase_ammo(10)  # Adjust the amount of health as needed
	queue_free()
